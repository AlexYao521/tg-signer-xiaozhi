...
> | PeriodicRoutine | ✅ | `periodic_tasks.py` | 已完成，包含元婴任务 |
> | StarObservation | ✅ | `star_observation.py` | 已完成，285行 |
> | HerbGarden | ✅ | `herb_garden.py` | 已完成，329行 |
> | YuanYingTasks | ❌ | ~~`yuanying_tasks.py`~~ | **已废弃**，功能已整合至 PeriodicRoutine |
...
### 4.6.1 元婴任务 (已整合至 PeriodicRoutine)
**注意：元婴任务已从独立模块移至 PeriodicRoutine (4.4)，作为周期任务管理。**

元婴出窍功能现在作为周期任务的一部分，遵循8小时冷却规则。
- `.元婴出窍` - 元婴出窍任务（8小时冷却）
- `.元婴状态` - 查询元婴状态（30分钟查询频率）

**历史实现说明（已废弃）：**
原独立的 YuanYingTasks 模块已被移除，其功能整合到 PeriodicTasks 中。
元婴相关的状态管理、冷却处理等现在统一由 PeriodicTasks 模块负责。
...
```jsonc
{
  ...
  "periodic": { "enable_biguan": true, "enable_qizhen": true, "enable_zhuzhen": true, "enable_wendao": true, "enable_yindao": true, "enable_yuanying": true, "enable_rift_explore": true },
...
| `periodic_state.json` | 各周期任务下次时间 (含裂缝和元婴) | `{ "acct_<a>_chat_<c>": {"qizhen": ts, "rift_explore": ts, "元婴出窍": ts}}` |
...
| 6 | ✅ | Periodic | 元婴任务整合 | 已移除独立YuanYingTasks模块，元婴出窍和状态查询已整合至PeriodicTasks |
...
### 15.1 分类顺序 (Pipeline)
Daily(SignIn/Greeting/Transmission) -> Star(观星台/收集/牵引/安抚) -> Herb(小药园/维护/播种/采药/兑换) -> Periodic(闭关/引道/问道/裂缝/启阵/助阵/元婴出窍/元婴状态) -> Activity(规则匹配) -> AI(普通对话)。
